...
void homog_transform(float x_particle, float y_particle, float heading_particle, float x_obs, float y_obs, float *x_map,float *y_map)
{

*x_map = x_particle + (COS(heading_particle)*x_obs) - (SIN(heading_particle)*y_obs);
*y_map = y_particle + (SIN(heading_particle)*x_obs) + (COS(heading_particle)*y_obs);
